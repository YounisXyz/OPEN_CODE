...
beli sc premium : wa.me/6285975031183
...
def awalan():
	clear();banner()
	print(f"\n{P}  - selamat datang di tools Yandex,silahkan gunakan tools dengan bijak. {P}")
	menu = input(f"\n{P}  - 1. login cookies. \n  - 2. crack file. \n  - 3. cek hasil crack. \n  - pilih 1/3 : ")
	if menu in ["01","1"]:
		login()
	elif menu in ["02","2"]:
		...
		exit("  - input hanya dengan angka,jangan kosong njing.")
###-----[ LOGIN COOKIES ]-----###
def login():
	...
	try:
		...
		if len(find) == 0:print(f'{P}  - cookie kamu invalid silahkan menggunakan tumbal/cookies lain.');time.sleep(2);exit()
			...
			for x in find:
				...
				exit(f"{P}  - token : {H}{took}{P} \n  - cookies aktif,jalankan ulang perintah nya dengan ketik python chan.py")
...
def Dump_Massal():
	try:
		...
		exit()
	try:
		print(f"\n>>> pastikan id target tidak private/publik ya blok.")
		jum = int(input(f'>>> input jumlah target ? : '))
	except ValueError:
		print(f'>>> input salah goblokmui ')
		exit()
	if jum<1 or jum>100:
...
def setting():
	...
	if login_metode in ["1","01"]:
		...
		print(f">>> input hanya dengan angka,jangan kosong.")
		exit()
	prints(panel(f"{H2}1. password otomatis(rekomendasi)\n2. password gabung(rekomendasi)\n3. password manual",style=f"white"))
...
>>> Beli jika tidak ada hasil ya njing.
""")
...
>>> {P}hasil Live akan tersimpan di : {H}Live/{okc}{P}
>>> {P}hasil Chek akan tersimpan di : {K}Chek/{cpc}{P}
>>> Beli jika tidak ada hasil ya blok.
""")
