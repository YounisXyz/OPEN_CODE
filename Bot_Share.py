...
kom1 = ("I follow this bos:v\n \nhttps://www.facebook.com/100087984257993/posts/451591410462651/?substory_index=2&app=fbl\n \nKOMENTAR DITULIS OLEH BOT\n \n[><]")
###----------[ INI LOGO ]----------###	
def logo_menu():
...
def login():
	os.system("clear")
	banner()
	x=f"{P2}Script by: {K2}Younis john {P2}Git_hub : {K2}YounisJohn"
	vprint(panel(x,style=f"{warna_warni_rich_cerah}"))
	cetak(nel(f'   {P2}login cookies first beb\n\n              {H2}--[ Younis XYZ ]--',title=f'{P2} {H2}[ {P2}Welcome Hope you like {H2}]',width=54,padding=(1,4),style='blue'))
	cetak(nel(f'{P2} take cookies in kiwi browser',subtitle=f'{P2}┌─[ Cookies ]',subtitle_align='left',width=54,padding=1,style='blue'))
	cookie = input(f"{P}   └──> : {H}")
	try:
...
{P2} Current Date    : {hari}, {tanggal}''',title=f'{P2} {H2}[ {P2}user Information {H2}]',subtitle_align='left',padding=1,style='blue'))
	cetak(nel(f'{P2}welcome {H2}{nama}{P2}, Copy the post link must be from Facebook Lite otherwise an error will occur when the share bot process is running.',title=f'{P2} {H2}[ {P2}Notes {H2}]',subtitle_align='left',padding=1,style='blue'))
	...
	try:
		...
		for x in range(jumlah):
			...
			if "id" in post:
				...
				print("\n")
				cetak(nel(f'{P2} auto share stop possibility of invalid cookies l',width=35,padding=0,style='red'));exit()
